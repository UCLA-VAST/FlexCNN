...
#pragma HLS DATA_PACK variable=fifo_transfer_out0
  ...
  case 1:{
    ...
    while(!done1){
      ...
      ap_uint<11> task_iter = 0;
      ap_uint<5> in_num_t = 0;
      bool done2 = 0;
      while(!done2){
...
#pragma HLS ARRAY_PARTITION variable=sel_tmp0 complete dim=1
      ...
      while(!done3){
#pragma HLS PIPELINE II=1
        ap_uint<5> feeder_id = t0 / U1_DATA1_FC_GROUP_FACTOR;
...
#pragma HLS DATA_PACK variable=fifo_transfer_in0
  ...
  while(!done1){
    ...
    ap_uint<5> t3 = 0;
    ap_uint<6> t1 = 0;
    ap_uint<6> t1_bound = LAYER_IN_IMG_H_T*LAYER_TCONV_STRIDE / LAYER_STRIDE;
    ap_uint<6> t0 = 0;
    ap_uint<4> t2 = 0;
    ap_uint<4> t2_bound = LAYER_IN_IMG_W_T*LAYER_TCONV_STRIDE / U1_SA_COLS / LAYER_STRIDE;
    bool done2 = 0;
    while(!done2){
#pragma HLS PIPELINE II=1
    ...
    while(!done1){
      ...
      if (in_num_iter >= LAYER_IN_NUM_HW){
        in_num_iter = 0;
        out_num_iter += LAYER_OUT_NUM_T;
        if (out_num_iter >= LAYER_OUT_NUM_HW){
          out_num_iter = 0;
          in_w_iter += LAYER_IN_W_T;
...
#pragma HLS INLINE off
  ...
  ap_uint<4> c0_counter = 0;
  ap_uint<1> c1_counter = 0;
  ap_uint<1> c2_counter = 0;
  ap_uint<3> c3_counter = 0;
  ap_uint<3> c4_counter = 0;
  ap_uint<3> c5_counter = 0;
  ap_uint<5> i = 0;

  ap_uint<6> c0_counter_bound;
  if (LAYER_STRIDE == 1){
...
#pragma HLS PIPELINE II=1
    ...
    ap_uint<10> w_bound = LAYER_COL_IL_FACTOR * LAYER_STRIDE + LAYER_FILTER_S_W - LAYER_STRIDE;
    ap_uint<10> h_bound = LAYER_IN_IMG_H_T + LAYER_FILTER_S_H - LAYER_STRIDE;
    ...
    ap_uint<10> wide_index = buffer_ind_to_feed_to_sys_arr / U1_DATA0_FC_SIMD_FACTOR + (K_W[0]-K_W[i]) + (K_H[0]-K_H[i]) * (LAYER_COL_IL_FACTOR+LAYER_FILTER_S_W-LAYER_STRIDE);
    ap_uint<10> wide_offset = buffer_ind_to_feed_to_sys_arr % U1_DATA0_FC_SIMD_FACTOR;
...
#pragma HLS ARRAY_PARTITION variable=sel_tmp_0 complete dim=1
  ...
  while(more_to_feed_to_sys_arr){
#pragma HLS PIPELINE II=1
    ap_uint<9> buffer_ind_to_feed_to_sys_arr;
    ...
    ap_uint<9> wide_index = buffer_ind_to_feed_to_sys_arr / U1_DATA1_FC_SIMD_FACTOR;
    ap_uint<9> wide_offset = buffer_ind_to_feed_to_sys_arr % U1_DATA1_FC_SIMD_FACTOR;
...
#pragma HLS INLINE off
  bool LAST_ENGINE = (engine_id == 6 / U1_DATA0_FC_SPLIT_FACTOR - 1);
  ap_uint<11> transfer_counter = 0;
  ap_uint<11> data0_buf_size;
  ap_uint<11> local_transfer_size;
  bool more_to_write_to_buffer = true;
  bool more_to_feed_to_sys_arr = false;
  bool more_to_forward = true;
  ap_uint<8> buffer_write_counter = 0;
  ...
  local_transfer_size = data0_buf_size * (6 / U1_DATA0_FC_SPLIT_FACTOR - engine_id) * U1_DATA0_FC_GROUP_FACTOR;
...
#pragma HLS PIPELINE II=1
    ...
    ap_uint<8> buffer_ind_to_write_to_buffer = buffer_write_counter;
...
#pragma HLS INLINE off
  bool LAST_ENGINE = (engine_id == 4 / U1_DATA1_FC_SPLIT_FACTOR - 1);
  ap_uint<9> transfer_counter = 0;
  ap_uint<9> data1_buf_size;
  ap_uint<9> local_transfer_size;
  bool more_to_write_to_buffer = true;
  bool more_to_feed_to_sys_arr = false;
  bool more_to_forward = true;
  ...
  local_transfer_size = data1_buf_size * (4 / U1_DATA1_FC_SPLIT_FACTOR - engine_id) * U1_DATA1_FC_GROUP_FACTOR;
...
#pragma HLS INLINE off
  ...
  bool LAST_ENGINE = (engine_id == 6 / U1_DATA2_FC_SPLIT_FACTOR - 1);
  ...
  ap_uint<3> oo = 0;
  ap_uint<6> h = 0;
  ap_uint<6> h_bound = LAYER_IN_IMG_H_T*LAYER_TCONV_STRIDE / LAYER_STRIDE;
  ap_uint<6> w = 0;
  ap_uint<6> w_bound = LAYER_IN_IMG_W_T*LAYER_TCONV_STRIDE / LAYER_STRIDE;
  LAYER_COL_IL_FACTOR *= LAYER_TCONV_STRIDE;
  bool done = 0;

  while(!done){
#pragma HLS PIPELINE II=1
    ap_uint<8> local_buf_idx = h * LAYER_COL_IL_FACTOR * LAYER_OUT_NUM_T + (w % LAYER_COL_IL_FACTOR) * LAYER_OUT_NUM_T + oo * U1_DATA2_FC_SIMD_FACTOR;
    if (w >= engine_id * LAYER_COL_IL_FACTOR){
      ap_uint<6> collector_id = w / LAYER_COL_IL_FACTOR;
...
#pragma HLS INLINE off
  ...
  bool more_to_collect_from_sys_arr = true;
  ap_uint<3> buffer_gs_id = 0;
  ap_uint<8> buffer_read_counter = 0;
  ap_uint<4> c0_counter = 0;
  ap_uint<1> c1_counter = 0;
  ap_uint<1> c2_counter = 0;
  ap_uint<3> c3_counter = 0;
  ...
  ap_uint<6> c0_counter_bound = LAYER_IN_IMG_H_T / LAYER_STRIDE;
...
#pragma HLS PIPELINE II=1
    ...
    ap_uint<8> buffer_ind_to_collect_from_sys_arr = offset0 + offset1 + offset2 + offset3;
...
void U1_DataCollect2Engine0(
  ...
  tapa::istream<U1_Data2PEChannelType> &fifo_collect_0,
  unsigned int engine_id,
  tapa::istream<uint> &fifo_config_in0, // from PE
  tapa::istream<uint> &fifo_config_in1, // from other engines
  tapa::ostream<uint> &fifo_config_out
){
#pragma HLS DATA_PACK variable=fifo_transfer_in
#pragma HLS DATA_PACK variable=fifo_transfer_out
...
void U1_DataCollect2Engine0_wrapper(
  ...
  tapa::istream<uint> &fifo_config_in1,
  tapa::ostream<uint> &fifo_config_out
){
