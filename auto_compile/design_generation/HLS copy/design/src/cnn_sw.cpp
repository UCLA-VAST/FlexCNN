...
void instInit(uint* config){
  ...
  config[4] = 0;
  if (in_file.is_open()){
    for (int layer_id = 0; layer_id < END_LAYER; layer_id++){
      uint p;
      int param_cnt = 0;
  ...
  // cout<<config[0]<<" "<<config[1]<<" "<<config[2]<<" "<<config[3]<<" "<<config[4]<<endl;
  // for (int layer_id = 0; layer_id < END_LAYER; layer_id++){
  //   for (int param_id = 0; param_id < CONFIG_PARAMS; param_id++){
  //     cout<<config[5+layer_id*CONFIG_PARAMS+param_id]<<" ";
  //   }
  //   cout<<endl;
  // }
  // exit(0);
...
void getInsts(uint* config, int start_layer, int end_layer){
  ...
  // cout<<config[0]<<" "<<config[1]<<" "<<config[2]<<" "<<config[3]<<" "<<config[4]<<endl;
  // for (int layer_id = 0; layer_id < config[0]; layer_id++){
  //   for (int param_id = 0; param_id < CONFIG_PARAMS; param_id++){
  //     cout<<config[5+layer_id*CONFIG_PARAMS+param_id]<<" ";
  //   }
  //   cout<<endl;
...
void preprocess(
  ...
  if (input2_file.is_open()){
    for (int i = in_num_hw*in_h_hw*in_w_hw; i < in_num_hw*in_h_hw*in_w_hw + 8*288*288; i++){
  ...
  // file_path = prj_path +"/data/bias.dat";
  ...
  for(int ch=0; ch<OUT_NUM; ch++){
    for(int h=0; h<OUT_H; h++){
      for(int w=0; w<OUT_W; w++){
        ...
        total_count++;
        if(abs(outputs_hw[ch][h][w]-outputs_sw[ch][h][w])>0.0001){
          flag = false;
