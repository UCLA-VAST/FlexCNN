...
def flexcnn_functions(arch_connectivity):
	...
	for module in arch_graph_nodes:
		if module == 'pool' or module == 'act_and_bn' or module == 'bias_load':
...
def engine_header(desp, config):
	code = []
	code.append('\n/**\n')
	code.append('You can add any new module that you want here.\n')
	code.append('You just need to follow the coding structure of the other modules.\n')
	code.append('You can uncomment the commented modules if your application needs them. All the modules listed in this file (commneted or uncommented) has a working implementation.\n')
	code.append('For convolution, you can either choose to use a naive implementation (which is slow), or add the systolic array kernel.\n')
	code.append('**/\n')
	code.append('void engine(\n')
	code.append('    bus_t0 *global_cin,\n')
	code.append('    bus_t0 *global_prev_cin,\n')
	code.append('    bus_t1 *global_weight,\n')
	code.append('    bus_t2 *global_bias,\n')
	code.append('    bus_t0 *global_cout,\n')
	code.append('    bus_t3 config[CONFIG_PARAMS],\n')
	code.append('    uint layer_id\n')
	code.append('){\n')
...
def top_function(desp, config):
	...
	code.append('			bus_t1 *global_weight,\n')
	code.append('			bus_t2 *global_bias,\n')
	code.append('			bus_t3 *layer_config,\n')
	code.append('			uint start_layer,\n')
	code.append('			uint end_layer\n')
	code.append('	){\n')
	code.append('	#pragma HLS INTERFACE m_axi port=global_cin offset=slave bundle=gmem1 depth=0\n')
	code.append('	#pragma HLS INTERFACE m_axi port=global_prev_cin offset=slave bundle=gmem3 depth=0\n')
	code.append('	#pragma HLS INTERFACE m_axi port=global_cout offset=slave bundle=gmem1 depth=826274\n')
	code.append('	#pragma HLS INTERFACE m_axi port=global_weight offset=slave bundle=gmem2 depth=34234\n')
	code.append('	#pragma HLS INTERFACE m_axi port=global_bias offset=slave bundle=gmem4 depth=1026\n')
	code.append('	#pragma HLS INTERFACE m_axi port=layer_config offset=slave bundle=gcontrol depth=2815\n')
	code.append('\n')
	code.append('	#pragma HLS INTERFACE s_axilite port=global_cin bundle=control\n')
	code.append('	#pragma HLS INTERFACE s_axilite port=global_prev_cin bundle=control\n')
	code.append('	#pragma HLS INTERFACE s_axilite port=global_weight bundle=control\n')
	code.append('	#pragma HLS INTERFACE s_axilite port=global_bias bundle=control\n')
	code.append('	#pragma HLS INTERFACE s_axilite port=global_cout bundle=control\n')
	code.append('	#pragma HLS INTERFACE s_axilite port=layer_config bundle=control\n')
	code.append('	#pragma HLS INTERFACE s_axilite port=return bundle=control\n')
	code.append('	#define DEBUG_layer\n')
	code.append('			// Copy the first instruction\n')
	code.append('		unsigned int init_inst[5]; 	// [LAYERS]\n')
	code.append('		memcpy((void*)init_inst, (void*)(&layer_config[0]), sizeof(unsigned int) * 5);\n')
	code.append('		int layers = init_inst[0];\n')
	code.append('\n')
	code.append('		int layer_num = layers;\n')
	code.append('		bus_t3 config[CONFIG_PARAMS * MAX_LAYER_BATCH];\n')
	code.append('		int cur_layer_batch = 1;\n')
	code.append('		int nxt_layer_batch = 1;\n')
	code.append('		int layer_id = start_layer;\n')
	code.append('		while(layer_id < end_layer){\n')
	code.append('			cur_layer_batch = nxt_layer_batch;\n')
	code.append('\n')
	code.append('			memcpy((void*)config, (void*)(&layer_config[5 + CONFIG_PARAMS * layer_id]), sizeof(unsigned int) * CONFIG_PARAMS * cur_layer_batch);\n')
	code.append('			nxt_layer_batch = config[CONFIG_PARAMS * (cur_layer_batch - 1) + 29 - 1];\n')
	code.append('			config[29 - 1] = cur_layer_batch;\n')
	code.append('				// call engine module for each of the layers\n')
	code.append('			engine(global_cin, global_prev_cin, global_weight, global_bias, global_cout, config, layer_id);\n')
	code.append('			#ifdef DEBUG_layer\n')
	code.append('				cout << "Passed " << layer_id + 1 << endl;\n')
	code.append('			#endif\n')
	code.append('				//layer_id += cur_layer_batch;\n')
	code.append('			layer_id += 1;\n')
	code.append('		}\n')
	code.append('	}\n')
	code.append('}\n')
	return code
...
def flexcnn_fifos(desp, config, arch_connectivity):
	...
	for node in sorted_graph:
		...
		for idx, edge in enumerate(out_edges):
			code.append(indent(1)+'hls::stream<CinLoadData0Type> %s_to_%s_%d("%s_to_%s_%d");\n'%(edge[0], edge[1], idx, edge[0], edge[1], idx))
			code.append(indent(1)+'#pragma HLS STREAM variable=%s_to_%s_%d depth=128\n'%(edge[0], edge[1], idx))
	...
	for idx in range(len(sorted_graph)-1):
		...
		code.append(indent(1)+'hls::stream<ConfigInst> config_%s_to_%s("config_%s_to_%s");\n'%(curr_node, next_node, curr_node, next_node))
		code.append(indent(1)+'#pragma HLS STREAM variable=config_%s_to_%s depth=16\n'%(curr_node, next_node))
...
def flexcnn_tasks(desp, config, arch_connectivity):
	...
	sorted_graph = list(nx.topological_sort(arch_graph))
	code.append(indent(1)+'layer_id += 1;\n')
	for idx, node in enumerate(sorted_graph):
		...
		# header
		code.append(indent(1)+'%s(\n'%node)
		
		# dram ports
		if node == 'cin_load':
			code.append(indent(1)+'		global_cin, \n')
			code.append(indent(1)+'		config,\n')
		elif node == 'cin_load_prev':
			code.append(indent(1)+'		global_prev_cin, \n')
			code.append(indent(1)+'		config_%s_to_%s,\n'%(prev_module, node))
		elif node == 'weight_load':
			code.append(indent(1)+'		global_weight, \n')
			code.append(indent(1)+'		config_%s_to_%s,\n'%(prev_module, node))
		elif node == 'bias_load':
			code.append(indent(1)+'		global_bias, \n')
			code.append(indent(1)+'		config_%s_to_%s,\n'%(prev_module, node))
		else:
			# inputs
		...
		# outputs
		if node == 'cout_write':
			code.append(indent(1)+'		global_cout \n')
		else:
			out_edges = arch_graph.out_edges(node)
			for idx, edge in enumerate(out_edges):
				code.append(indent(1)+'		%s_to_%s_%d, \n'%(edge[0], edge[1], idx))
		...
		code.append(indent(1) + '#ifdef DEBUG_engine\n')
		code.append(indent(1) + f'	cout << "passed {node}" << endl;\n')
		code.append(indent(1) + '#endif\n')
		code.append(indent(1) + f'#ifdef DEBUG_{node}\n')
		code.append(indent(1) + 'if(layer_id==TARGET_INST){\n')
		code.append(indent(1) + '	string prj_path = string(getenv("PRJ_PATH"));\n')
		code.append(indent(1) + '	string layer_id_str = to_string(layer_id);\n')
		out_edges = arch_graph.out_edges(node)
		for idx, edge in enumerate(out_edges):
			if idx == 0:
				code.append(indent(1) + f'	string file_path = prj_path + "/data/test/{node}_{idx}_"+layer_id_str+".dat";\n')
				code.append(indent(1) + '	int count = 0;\n')
				code.append(indent(1) + '	float sum = 0;\n')
				code.append(indent(1) + '	FILE *f;\n')
			else:
				code.append(indent(1) + f'	file_path = prj_path + "/data/test/{node}_{idx}_"+layer_id_str+".dat";\n')
				code.append(indent(1) + '	count = 0;\n')
				code.append(indent(1) + '	sum = 0;\n')
			code.append(indent(1) + '	f = fopen(file_path.c_str(), "w");\n')
			code.append(indent(1) + '	while(!%s_to_%s_%d.empty()){\n'%(edge[0], edge[1], idx))
			code.append(indent(1) + '		ReluData0Type item = %s_to_%s_%d.read();\n'%(edge[0], edge[1], idx))
			code.append(indent(1) + '		data_t2 num[8];\n')
			code.append(indent(1) + '		for(int i=0; i<SIMD_LANE; i++){\n')
			code.append(indent(1) + '			num[i] = Reinterpret<data_t2>((ap_uint<32>)item((i+1)*32-1, 32*i));\n')
			code.append(indent(1) + '			fprintf(f, "%10f\t", num[i]);\n')
			code.append(indent(1) + '			count++;\n')
			code.append(indent(1) + '			sum += num[i];\n')
			code.append(indent(1) + '		}\n')
			code.append(indent(1) + '		fprintf(f, "\\n");\n')
			code.append(indent(1) + '	}\n')
			code.append(indent(1) + '		fprintf(f, "sum: %f\\n", sum);\n')
			code.append(indent(1) + '		fprintf(f, "count: %d\\n", count);\n')
			code.append(indent(1) + '	cout<<"sum: "<<sum<<endl;\n')
			code.append(indent(1) + '	cout<<"count: "<<count<<endl;\n')
		code.append(indent(1) + '	fclose(f);\n')
		code.append(indent(1) + '	exit(0);\n')
		code.append(indent(1) + '}\n')
		code.append(indent(1) + '#endif\n')
...
#    code.append(indent(1) + 'fifo_config_out.write(LAYER_OUT_IMG_W);\n')
		...
		code.append(indent(3) + 'local_reg_id++;\n')
		code.append(indent(3) + 'if (local_reg_id == (i+1)*LAYER_LOCAL_REG_NUM){\n')
		code.append(indent(4) + 'local_reg_id = i*LAYER_LOCAL_REG_NUM;\n')
		code.append(indent(4) + 'la_counter++;\n')
		code.append(indent(4) + 'if (la_counter == LAYER_LOCAL_ACCUM_NUM_ARR[i]){\n')
		code.append(indent(5) + 'la_counter = 0;\n')
		code.append(indent(5) + 'i++;\n')
		code.append(indent(5) + 'local_reg_id = i*LAYER_LOCAL_REG_NUM;\n')
...
#      code.append(indent(1) + 'for (ap_uint<%d> layer_iter = 0; layer_iter < LAYER_BATCH; layer_iter++){\n' % (w))
			...
			code.append(indent(3) + 'inst5 = fifo_kernel_config_in.read();\n')
			code.append(indent(3) + 'fifo_kernel_config_out.write(inst5);\n')
			code.append(indent(2) + '}\n')
			...
			code.append(indent(2) + 'ap_uint<8>  EXT_FILTER_S2_H      = inst2(32*4+23, 32*4+16);\n')
			code.append(indent(2) + 'ap_uint<8>  EXT_FILTER_S2_W      = inst2(32*4+31, 32*4+24);\n')
			...
			code.append(indent(2) + 'ap_uint<16> EXT_LAYER_CONV_TYPE     = inst5(32*0+15, 32*0);\n')
			code.append(indent(2) + 'ap_uint<8>  EXT_FILTER_D0_H         = inst5(32*0+23, 32*0+16);\n')
			code.append(indent(2) + 'ap_uint<8>  EXT_FILTER_D0_W         = inst5(32*0+31, 32*0+24);\n')
			code.append(indent(2) + 'ap_uint<8>  EXT_FILTER_D1_H         = inst5(32*1+7, 32*1);\n')
			code.append(indent(2) + 'ap_uint<8>  EXT_FILTER_D1_W         = inst5(32*1+15, 32*1+8);\n')
			code.append(indent(2) + 'ap_uint<16> EXT_LAYER_DILATION_RATE = inst5(32*1+31, 32*1+16);\n')
			code.append(indent(2) + 'ap_uint<16> EXT_LAYER_TCONV_STRIDE  = inst5(32*2+15, 32*2);\n')
			code.append(indent(2) + 'ap_uint<16> EXT_K_NUM               = inst5(32*2+31, 32*2+16);\n')
			code.append(indent(2) + 'ap_uint<32> EXT_KH                  = inst5(32*3+31, 32*3);\n')
			code.append(indent(2) + 'ap_uint<32> EXT_KW                  = inst5(32*4+31, 32*4);\n')
			...
			code.append(indent(2) + '}\n')
			code.append(indent(2) + 'uint LAYER_FILTER_S_H = EXT_FILTER_S2_H;\n')
			code.append(indent(2) + 'uint LAYER_FILTER_S_W = EXT_FILTER_S2_W;\n')
...
#      code.append(indent(2) + 'uint LAYER_STRIDE = EXT_STRIDE;\n\n')
			...
			code.append(indent(2) + 'uint LAYER_CONV_TYPE = EXT_LAYER_CONV_TYPE;\n')
			code.append(indent(2) + 'uint FILTER_D0_H = EXT_FILTER_D0_H;\n')
			code.append(indent(2) + 'uint FILTER_D0_W = EXT_FILTER_D0_W;\n')
			code.append(indent(2) + 'uint FILTER_D1_H = EXT_FILTER_D1_H;\n')
			code.append(indent(2) + 'uint FILTER_D1_W = EXT_FILTER_D1_W;\n')
			code.append(indent(2) + 'uint LAYER_DILATION_RATE = EXT_LAYER_DILATION_RATE;\n')
			code.append(indent(2) + 'uint LAYER_TCONV_STRIDE = EXT_LAYER_TCONV_STRIDE;\n')
			code.append(indent(2) + 'uint K_NUM = EXT_K_NUM;\n')
			code.append(indent(2) + 'ap_uint<32> KH = EXT_KH;\n')
			code.append(indent(2) + 'ap_uint<32> KW = EXT_KW;\n')
