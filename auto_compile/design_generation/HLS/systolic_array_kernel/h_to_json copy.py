...
def get_mem_params(inst_file, insts_num):
	...
	max_upsample_factor = 0
	max_downsample_factor = 0
	max_in_num = 0
	max_out_num = 0
	max_in_h = 0
	max_in_w = 0
	...
	for inst in instDicts:
		...
		if upsample_factor > max_upsample_factor:
			max_upsample_factor = upsample_factor
		if downsample_factor > max_downsample_factor:
			max_downsample_factor = downsample_factor
		...
		if out_w > max_out_w:
			max_out_w = out_w
		if in_num_t > max_in_num_t:
			max_in_num_t = in_num_t
		if out_num_t > max_out_num_t:
			max_out_num_t = out_num_t
		if in_h_t > max_in_h_t:
			max_in_h_t = in_h_t
		if in_w_t > max_in_w_t:
			max_in_w_t = in_w_t
		if out_h_t > max_out_h_t:
			max_out_h_t = out_h_t
		if out_w_t > max_out_w_t:
			max_out_w_t = out_w_t
		if filter_d0_h > max_filter_d0_h:
			max_filter_d0_h = filter_d0_h
		if filter_d0_w > max_filter_d0_w:
			max_filter_d0_w = filter_d0_w
		if filter_d1_h > max_filter_d1_h:
			max_filter_d1_h = filter_d1_h
		if filter_d1_w > max_filter_d1_w:
			max_filter_d1_w = filter_d1_w
		...
		data2_buf_size = out_num_t*out_h_t*col_il_factor*upsample_factor
	...
	max_buff_dict['IN_NUM'] = max_in_num
	max_buff_dict['OUT_NUM'] = max_out_num
	max_buff_dict['IN_H'] = max_in_h
	max_buff_dict['IN_W'] = max_in_w
	max_buff_dict['OUT_H'] = max_out_h
	max_buff_dict['OUT_W'] = max_out_w
	max_buff_dict['IN_NUM_T'] = max_in_num_t
	max_buff_dict['OUT_NUM_T'] = max_out_num_t
	max_buff_dict['IN_H_T'] = max_in_h_t
	max_buff_dict['IN_W_T'] = max_in_w_t
	max_buff_dict['OUT_H_T'] = max_out_h_t
	max_buff_dict['OUT_W_T'] = max_out_w_t
	max_buff_dict['FILTER_D0_H'] = max_filter_d0_h
	max_buff_dict['FILTER_D0_W'] = max_filter_d0_w
	max_buff_dict['FILTER_D1_H'] = max_filter_d1_h
	max_buff_dict['FILTER_D1_W'] = max_filter_d1_w
	max_buff_dict['STRIDE'] = max_stride
...
if __name__ == "__main__":
	...
	K = max(buff_dict['FILTER_D0_H'], buff_dict['FILTER_D0_W'], buff_dict['FILTER_D1_H'], buff_dict['FILTER_D1_W'])
	TS = design_params['TSTRIDE']
	DR = design_params['DILATION_RATE']
	IN_NUM = buff_dict['IN_NUM']
	OUT_NUM = buff_dict['OUT_NUM']
	IN_IMG_H = buff_dict['IN_H'] + K -1
	IN_IMG_W = buff_dict['IN_W'] + K -1

	IN_NUM_T = buff_dict['IN_NUM_T']
	OUT_NUM_T = buff_dict['OUT_NUM_T']
	IN_H_T = buff_dict['IN_H_T']
	IN_W_T = buff_dict['IN_W_T']
	OUT_H_T = IN_H_T*TS
	OUT_W_T = IN_W_T*TS

	IN_NUM_T_MAX = IN_NUM_T
	OUT_NUM_T_MAX = OUT_NUM_T
	IN_H_T_MAX = IN_H_T
	IN_W_T_MAX = IN_W_T
	...
	with open(prj_path + '/auto_compile/design_generation/HLS/systolic_array_kernel/cnn_features.json', 'w') as outfile:
	...
	paramfile.append('#define CIN_SIZE 14193472\n')
	paramfile.append('#define WEIGHT_SIZE 560064\n')
	paramfile.append('#define BIAS_SIZE 16544\n')
...
#define BIAS_SIZE 16544
	...
	#print jsonfile to out.json
	with open(prj_path + '/auto_compile/design_generation/HLS/design/src/params.h', 'w') as outfile:
