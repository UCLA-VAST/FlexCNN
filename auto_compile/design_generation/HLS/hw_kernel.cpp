...
void weight_load(
  ...
  // int count = 0;
  // while (!fifo_conv_weight.empty()){
  //   fifo_conv_weight.read();
  //   count++;
  // }
  // cout<<"count: "<<count*SIMD_LANE<<endl;
...
void concat(
		...
		hls::stream<ConfigInst>           &fifo_config_in,
		hls::stream<ConvData0Type>        &fifo_cout_0,
    hls::stream<ConvData0Type>        &fifo_cout_1,
		hls::stream<ConfigInst>           &fifo_config_out
){
	#pragma HLS INLINE off
		// tiling iterators
  ...
  int count = 0;
	...
	#pragma HLS PIPELINE II=1
					// if(!fifo_conv.empty()/* && !fifo_cin.empty()*/){
						ConvData0Type tmp = 0;
            if (!fifo_cin.empty()){
              fifo_cin.read_nb(tmp);
              fifo_cout_0.write(tmp);
            }
            if (!fifo_conv.empty()){
              fifo_conv.read_nb(tmp);
              fifo_cout_1.write(tmp);
            }
	...
	#pragma HLS PIPELINE II=1
		...
		case 1:
			// if ((max_pool && out_num_iter == 0) || (!max_pool && (in_num_iter + LAYER_IN_NUM_T >= POOL_NUM_HW))){
				maxpool_w2 <data_t0, MAX_OUT_H_T, MAX_OUT_W_T, POOL_LANE, 2, DATA_W0> (fifo_cin, fifo_cout, STRIDE, POOL_EN, LAYER_NUM_T, LAYER_IN_H_T, LAYER_IN_W_T);
			// }
			break;
		}
...
void engine(
	...
	#pragma HLS STREAM variable=fifo_pool_0 depth=64
  	// Module: concat
		// Output ports:
		// 0: cout -> inter_write
	hls::stream<PoolData0Type> fifo_concat_0;
	#pragma HLS STREAM variable=fifo_concat_0 depth=64
  hls::stream<PoolData0Type> fifo_concat_1;
	#pragma HLS STREAM variable=fifo_concat_1 depth=64
  	// Module: upsample
		// Output ports:
		// 0: cout -> data_write
  ...
  hls::stream<ConfigInst> config_concat;
  ...
  #pragma HLS STREAM variable=config_concat depth=16
...
void engine(
  ...
  if(layer_id==TARGET_INST){
    ...
    string file_path = prj_path + "/data/test/cin_"+layer_id_str+".dat";
    FILE *f;
    ...
    while(!fifo_cin_load_0.empty()){
      ...
      for(int i=0; i<SIMD_LANE; i++){
        num[i] = Reinterpret<data_t2>((ap_uint<DATA_W0>)item((i+1)*DATA_W0-1, DATA_W0*i));
        fprintf(f, "%10f\t", num[i]);
        count++;
        sum += num[i];
      }
    ...
    cout<<"cin count: "<<count<<endl;
    cout<<"cin sum: "<<sum<<endl;
    fclose(f);
    exit(0);
  }
	#endif
...
// cin sum: -41.6485
	...
	#ifdef DEBUG_CIN_PREV
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/cin_prev_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
    float sum = 0;
    ...
    cout<<"cin_prev count: "<<count<<endl;
    cout<<"cin_prev sum: "<<sum<<endl;
    fclose(f);
    exit(0);
  }
	#endif
  ...
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/beta_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
    float sum = 0;
    while(!fifo_beta_conv.empty()){
      ReluData0Type item = fifo_beta_conv.read();
      data_t2 num[8];
      for(int i=0; i<8; i++){
    ...
    cout<<"count: "<<count<<endl;
    fclose(f);
    file_path = prj_path + "/data/test/gamma_"+layer_id_str+".dat";
    f = fopen(file_path.c_str(), "w");
    count = 0;
    sum = 0;
    while(!fifo_gamma_conv.empty()){
      ReluData0Type item = fifo_gamma_conv.read();
      data_t2 num[8];
      for(int i=0; i<8; i++){
        num[i] = Reinterpret<data_t2>((ap_uint<32>)item((i+1)*32-1, 32*i));
        fprintf(f, "%10f\t", num[i]);
  ...
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/weights_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
  ...
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/conv_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
    float sum = 0;
  ...
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/upsample_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
    float sum = 0;
    while(!fifo_upsample_0.empty()){
      ReluData0Type item = fifo_upsample_0.read();
      data_t2 num[SIMD_LANE];
      for(int i=0; i<SIMD_LANE; i++){
        num[i] = Reinterpret<data_t2>((ap_uint<32>)item((i+1)*32-1, 32*i));
        fprintf(f, "%10f\t", num[i]);
  ...
  if(layer_id==TARGET_INST){
    // string prj_path = string(getenv("PRJ_PATH"));
    // string layer_id_str = to_string(layer_id);
    // string file_path = prj_path + "/data/test/pool_"+layer_id_str+".dat";
    // FILE *f;
    // f = fopen(file_path.c_str(), "w");
    int count = 0;
    // float sum = 0;
  ...
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/concat_0_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
    float sum = 0;
    while(!fifo_concat_0.empty()){
      ReluData0Type item = fifo_concat_0.read();
      data_t2 num[8];
      for(int i=0; i<8; i++){
        num[i] = Reinterpret<data_t2>((ap_uint<32>)item((i+1)*32-1, 32*i));
        fprintf(f, "%10f\t", num[i]);
    ...
    cout<<"count: "<<count<<endl;
    fclose(f);
    file_path = prj_path + "/data/test/concat_1_"+layer_id_str+".dat";
    f = fopen(file_path.c_str(), "w");
    count = 0;
    sum = 0;
    while(!fifo_concat_1.empty()){
      ReluData0Type item = fifo_concat_1.read();
      data_t2 num[8];
      for(int i=0; i<8; i++){
        num[i] = Reinterpret<data_t2>((ap_uint<32>)item((i+1)*32-1, 32*i));
        fprintf(f, "%10f\t", num[i]);
  ...
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/add_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
    float sum = 0;
  ...
  if(layer_id==TARGET_INST){
    string prj_path = string(getenv("PRJ_PATH"));
    string layer_id_str = to_string(layer_id);
    string file_path = prj_path + "/data/test/relu_"+layer_id_str+".dat";
    FILE *f;
    f = fopen(file_path.c_str(), "w");
    int count = 0;
    float sum = 0;
    ...
    cout<<"relu count: "<<count<<endl;
    cout<<"relu sum: "<<sum<<endl;
    fclose(f);
    exit(0);
  }
	#endif
