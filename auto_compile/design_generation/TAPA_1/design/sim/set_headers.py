...
def setHeaders(prj_path, start_layer, end_layer, layer_id_test, target_module, sl_option, prev_run_str):
...
#define END_LAYER 1
  ...
  for line in inFile:
    ...
    if(len(values)!=0):
      if(values[0]=="#define" and values[1]=="LAYER_NUM"):
        ...
        if(((instDict['OUT_W_HW'] == instDict['OUT_W']) or (instDict['OUT_W_HW'] == instDict['IN_W_T'])) and ((instDict['OUT_H_HW'] == instDict['OUT_H']) or (instDict['OUT_H_HW'] == instDict['IN_H_T'])) and 'UNet' not in str(prj_path)):
  ...
  first_line = tempLines[0]
  if prev_run_str!=first_line:
    outFile = open(prj_path / 'src' / 'cnn_sw.h', "w")
    outFile.writelines(prev_run_str)
    for line in tempLines[1:]:
      outFile.writelines(line)
    outFile.close()
    inFile.close()
