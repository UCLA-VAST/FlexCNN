...
void maxpool_w2(
  tapa::istream<ap_uint<T_DATA_WIDTH0 * T_UNROLL> > &fifo_in,
  tapa::ostream<ap_uint<T_DATA_WIDTH0 * T_UNROLL> > &fifo_out,
  uint                                            stride,  
  bool                                            max_en,
...
#pragma HLS PIPELINE II=1    
    ...
    for (int dup = 0; dup < T_UNROLL; dup++){
      ...
      T_data_t0 tmp1 = 0;
      // line_buf1[dup][layer_in_w_t - 1];
      // add a mux to support dynamic tiling
      if (layer_in_w_t == MAX_IN_W_T) {
        tmp1 = line_buf1[dup][MAX_IN_W_T-1];
      } else if (layer_in_w_t == MAX_IN_W_T/2) {
        tmp1 = line_buf1[dup][MAX_IN_W_T/2-1];
      } else if (layer_in_w_t == MAX_IN_W_T/4) {
        tmp1 = line_buf1[dup][MAX_IN_W_T/4-1];
      } else if (layer_in_w_t == MAX_IN_W_T/8){
        tmp1 = line_buf1[dup][MAX_IN_W_T/8-1];
      }
