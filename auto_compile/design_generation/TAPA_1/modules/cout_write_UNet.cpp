  ...
  // cout<<"write: "<<write<<endl;
	...
	if (run){
      ...
      for (int hh = 0; hh < LAYER_IN_H_T / 2; hh++){
				...
				uint global_cout_idx = num_iter / LAYER_OUT_NUM_T * (LAYER_OUT_H_HW-2) * (LAYER_OUT_W_HW-2) * LAYER_OUT_NUM_T + h * (LAYER_OUT_W_HW-2) * LAYER_OUT_NUM_T + in_w_iter / 2 * LAYER_OUT_NUM_T + cout_offset;
      ...
      cout_offset += (LAYER_OUT_W_HW-2)*(LAYER_OUT_H_HW-2)*LAYER_OUT_NUM + ((LAYER_OUT_W_HW-2)+3)*LAYER_OUT_NUM_T;
			for (int hh = 0; hh < LAYER_IN_H_T / 2; hh++){
				uint h = in_h_iter / 2 + hh;
...
void cout_write(
  ...
  int count = 0;
    ...
    // cout<<LAYER_OUT_W_HW<<" "<<LAYER_OUT_W<<endl;
		...
		change_layout = 0;//(((LAYER_OUT_W_HW == LAYER_OUT_W) || (LAYER_OUT_W_HW == LAYER_IN_W_T)) && ((LAYER_OUT_H_HW == LAYER_OUT_H) || (LAYER_OUT_H_HW == LAYER_IN_H_T))); 	// if next filter = 1 : change the layout to num_tile, h_t, w_t, in_t
