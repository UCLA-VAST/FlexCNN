void act_and_bn(
  ...
  int count = 0;
    ...
    ap_uint<32> FILTER_D0           = inst5(32*0+23, 32*0+16);
    ...
    // cout<<RELU_EN<<" "<<BIAS_EN<<" "<<BATCH_NORM_EN<<endl;
		...
		data_t2 beta_buf[MAX_IN_NUM_T / RELU_LANE][RELU_LANE];
		data_t2 gamma_buf[MAX_IN_NUM_T / RELU_LANE][RELU_LANE]; 
    ...
    // cout<<en<<endl;
		...
		case 0:{
			...
			if (((max_pool || UP_SAMPLE_EN) && out_num_iter == 0) || (!max_pool && (out_num_iter + LAYER_OUT_NUM_T <= LAYER_OUT_NUM))){
				...
				int w_bound = (LAYER_IN_W_T / stride + FILTER_D0 - 1);
				int h_bound = (LAYER_IN_H_T / stride + FILTER_D0 - 1);
...
#pragma HLS PIPELINE II=1
          ConvData0Type beta = 0;
          ConvData0Type gamma = 0;
          if(BATCH_NORM_EN){
...
#pragma HLS UNROLL    
            ...
            if(BATCH_NORM_EN)
              tmp = 0.9995003746640602*cin_data;
            if(RELU_EN)
              tmp = max(tmp*0.0010000000474974513, tmp);
