...
def get_en_value(inst):
	...
	# print(f'conv_en: {conv_en}, relu_en: {relu_en}, pool_en: {pool_en}, upsample_en: {upsample_en}, bias_en: {bias_en}, bn_en: {bn_en}, prev_en: {prev_en}, cnct_en: {cnct_en}, add_en: {add_en}')
	return en
...
def calculate_addresses(f_model, insts, sa_rows, sa_cols, sa_simd):
	...
	if 'ENet' in f_model:
		...
		data = {}
		data['main_output_addr'] = 672800*2
	...
	for idx, inst in enumerate(insts):
		...
		for main_input in inst['main_input']:
			if idx == 0 and 'ENet' in f_model:
				...
				graph.add_edge(main_input+'_padded', inst['name'], data = data)
				data = {'tensor_size': (out_num, out_h, out_w), 'type': 'secondary'} # secondary with respect to the receiving node
				graph.add_edge('StatefulPartitionedCall/model/initial_max_pooling_layer/MaxPool__6', inst['name'], data = data)
			else:
				data = {'tensor_size': (out_num, out_h, out_w), 'type': 'main'} # main with respect to the receiving node
	...
	for idx, inst in enumerate(insts):
		...
		if idx == 0 and 'ENet' in f_model:
			data['main_output_addr'] = 16*290*290+16*288*288
		...
		if idx == 72 and 'ENet' in f_model:
			# data['main_output_addr'] = 8999904 + 64*72*72 + 16*39#9354784
			data['main_output_addr'] = 10336256 + 64*72*72 + 16*39
		if idx == 82 and 'ENet' in f_model:
			# data['main_output_addr'] = 10774176 + 16*144*144 + 16*75#11107152
			data['main_output_addr'] = 12110528 + 16*144*144 + 16*75
...
def run(f_model, f_arch, f_insts):
	...
	with open(f_model, 'r') as f:
		...
		for idx, in_inst in enumerate(raw_insts):
			inst = {}
