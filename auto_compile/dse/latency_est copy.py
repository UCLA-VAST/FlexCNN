...
def model_latency_est(params, layer_configs, layers_order, dynamic_tiling_level):
	...
	# print('----------------------------------------------------------------------------------------')
	for layer_name in layers_order:
		hw_sw_params = params.copy()
		layer_config = layer_configs[layer_name]
		hw_sw_params.update(layer_config)
		
		in_num_hw = params['LAYER_IN_NUM_T']
		out_num_hw = params['LAYER_OUT_NUM_T']
		in_h_hw = params['LAYER_IN_H_T']
		in_w_hw = params['LAYER_IN_W_T']
		out_h_hw = params['LAYER_OUT_H_T']
		out_w_hw = params['LAYER_OUT_W_T']
		...
		if dynamic_tiling_level == 0 or dynamic_tiling_level == 1:
			...
			for depend_layer_name in layer_in_w_dependency:
				...
				if visited_layers[depend_layer_name]:
					...
					flag = False
			if flag:
				layer_in_w_t_candidates = list(filter(lambda x : x % sa_cols == 0 and layer_in_w % x == 0, range(1, layer_in_w + 1)))
	...
	params['LAYER_DSP_EFF_LIST'] = layer_dsp_eff_list
	params['MODEL_FAIL'] = model_fail
